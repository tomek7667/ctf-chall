...
window.addEventListener("load", () => {
	...
	const parseUrl = (url) => {
		return new URL(url).host.endsWith(".jsapi.live");
	};
	...
	const self = new (class {
		...
		set(str) {
			...
			var bookmarkName = DOMPurify.sanitize(str, {
				ADD_TAGS: ["link", "style"],
			}); // allow CSS
		...
		get() {
			return (
				console.log("NOTE_APP_GETTER_CALL"),
				this.note ||
					parsed.getAttribute("data-last") ||
	...
	return (
		(("true" === urlInstance.searchParams.get("enableapi") &&
			parseUrl(urlInstance.searchParams.get("recv")) &&
			window.parent) ||
			window.opener) &&
			(onmessage("NOTE_APP_API_LOADED"),
			(window.a = urlInstance.searchParams.get("recv")),
				...
				if (parseUrl(event.origin)) {
					if ("string" == typeof event.data) {
						if (event.data.startsWith("NOTE_APP_FLAG_REQUEST")) {
							...
							factor_text = (await fetch("file:///flag.txt"))
								.text;
							if (!(event.source === window)) {
								onmessage("You need to try a bit harder...");
							}
							onmessage(
								"NOTE_APP_FLAG_REQUEST_RESPONSE " + factor_text
							);
						} else {
							if (event.data.startsWith("NOTE_APP_SET_REQUEST")) {
								onmessage(
									"NOTE_APP_EXPERIMENTAL_API_CALL_MADE "
								);
								self.set(event.data.split(" ")[1]);
							}
						}
					}
				} else {
